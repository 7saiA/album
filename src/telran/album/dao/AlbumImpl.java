...
public class AlbumImpl implements Album {
    ...
    public AlbumImpl(int capacity) {photos = new Photo[capacity];}
    ...
    public boolean removePhoto(int photoId, int albumId) {
        for (int i = 0; i < size; i++) {
            if(photos[i].getPhotoId() == photoId && photos[i].getAlbumId() == albumId) {
                System.arraycopy(photos,i,photos,i+1,photos.length-1-i);
                photos[--size] = null;
                return true;
            }
        }
    ...
    public Photo[] getPhotoBetweenDate(LocalDate dateFrom, LocalDate dateTo) {
        return findPhotoByPredicate(e -> (e.getDate().toLocalDate().isEqual(dateFrom)
                || e.getDate().toLocalDate().isAfter(dateFrom))
                && e.getDate().toLocalDate().isBefore(dateTo));
    }
